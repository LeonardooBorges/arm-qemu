...
#define SLEEP_SYSCALL_NB	2
#define WAKEUP_SYSCALL_NB	3
#define GETLINHA_SYSCALL_NB	4
#define PUTLINHA_SYSCALL_NB	5
...
extern void syscall_escreve_linha(UART *up, u8 *s);
...
void process()
{
	...
	while(1) 
	{
		...
		t = trata_linha(pid,s);
		// uprints(&uart[0],t);
		system_call(PUTLINHA_SYSCALL_NB, (int) t);
    }
}
...
int SWI_Handler_C(int nb_syscall, int param)
{
	...
	else if (nb_syscall == PUTLINHA_SYSCALL_NB){
        syscall_escreve_linha(&uart[0], (char *) param);
        return 1;
    } 
}
