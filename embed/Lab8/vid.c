...
*/
void show_bmp(u32 start_row, u32 start_col, int x, int y)
{
    u32 h, w, pixel, rsize, i, j;
    u8 r, g, b;
    for(i = start_row; i<start_row + gDisplayContext.screen_height/2; i++)
    {
        for(j=start_col+ x*gDisplayContext.screen_width/4;j<start_col+ (x+1)*gDisplayContext.screen_width/4;j++)
        {
	    if(y == 1) {
		b = 0;g=0;r=0;
	    }
