...
Reset_Handler:
    ...
    MSR     cpsr, r2                            ;@ volta pro modo anterior
    BIC     r1, r1, #0x80                       ;@ enable interrupts in the cpsr
    STR     r1, [r0, #64]                       ;@ cpsr = 16*4 = 64

    MSR     cpsr, r12                           ;@ volta para o modo anterior
...
Undefined_Handler:
    ...
    LDR     lr, =nproc                          ;@ load addr nproc
    LDR     r0, [lr]                            ;@ r0 =  1, 2 ou 3 (processo corrente)
    ADD     r0, r0, #48
    BL      undefined
    LDMFD   sp!, {r0-r12,pc}^
...
Timer_Handler:
    ...
    MRS     r0, cpsr                            ;@ salvando o modo corrente em R0
    LDR     r11, =nproc                          ;@ load addr nproc
    LDR     r12, [r11]                            ;@ r1 =  1, 2 ou 3 (processo corrente)
    CMP     r12, #2
    MSREQ   cpsr_ctl, #0b11010111
    MSRNE   cpsr_ctl, #0b11010011               ;@ alterando o modo para supervisor
    ...
    ADD     r1, r1, #1                          ;@ r1++
    SUB     r2, r1, #3                          ;@ compara r1 com qtd de processos + 1 (caso aumente n processos alterar aqui)
    CMP     r2, #0                              
